...
plot3 = data |> 
  ...
  mutate(`Group Size` = case_when(`Group Size` == "logorG1" ~ "Sample Size: 800",
                                  `Group Size` == "logorG2" ~ "Sample Size: 100")) |> 
  ggplot(aes(colour = `Group Size`,
             fill = `Group Size`,
             x = `Log Odds Ratio`)) +
  facet_wrap(vars(`Group Size`), nrow = 2, ncol = 1) +
  ...
  labs(title = NULL,
       ...
       y = NULL) +
  # Themeing
  scale_fill_discrete(type = c("#377eb8", "#e41a1c")) +
  scale_colour_discrete(type = c("#377eb8", "#e41a1c")) +
  theme_light() +
  theme(legend.title = element_blank(),
        legend.position = "bottom",
        # Anaesthesia themeing
...
# Save images
ggsave("outputs/plot2.jpeg", plot = plot2,
       dpi = 600, width = 6, height = 9, units = "in")

ggsave("outputs/plot3.jpeg", plot = plot3,
       dpi = 600, width = 6, height = 9, units = "in")