...
## Exclude cardiac surgery and thoracic transplant
data.list = data.list %>%
    map(filter,
        !dx_primary %in% c("dx_card_heartx", "dx_resp_lungtx",
                           "dx_card_simple", "dx_card_complex", "dx_card_congenital", "dx_card_vasc")) %>%
    map(droplevels)
...
saveRDS(data.list, "sensitivity/matched-data.list.Rds")
rm(trim)
...
# VIF ranges 1.2 to 9.3 if NZ is included
# VIF ranges 1.2 to 1.55 if NZ is excluded (see line 295)
...
# Range 1.78 to 1.81 with NZ included
# Range 1.79 to 1.81 with NZ excluded (see line 303)
...
plot.forest = forest(data = tab.forest,
                     ...
                     xlim = c(0, 3.2),
                     ticks_at = seq(0.2, 3.2, 0.4),
                     ci_column = 3) %>%
    add_border(part = "header",
               where = "bottom")

ggsave("sensitivity/figures/forest.jpg", plot.forest,
       dpi = 600, width = 12, height = 6, units = "in")


### Complete case analysis - demonstrates more extreme ORs in both directions, without change in directionality from MI model
reg.multi.cc = select %>%
    mutate(ecmo_centre_type = ecmo_centre_type %>%
           ...
           remoteness_cat = remoteness_cat %>%
               ...
               relevel("Major City") %>%
               fct_relevel("NZ", after = Inf)) %>%
    filter(!dx_primary %in% c("dx_card_heartx", "dx_resp_lungtx",
                              "dx_card_simple", "dx_card_complex", "dx_card_congenital", "dx_card_vasc")) %>%
...
ggsave("sensitivity/figures/forest-no-nz.jpg", plot.nz.forest,
       dpi = 600, width = 12, height = 6, units = "in")
...
ggsave("sensitivity/figures/remote.jpg", plot.remote,
       dpi = 600, width = 12, height = 6, units = "in")