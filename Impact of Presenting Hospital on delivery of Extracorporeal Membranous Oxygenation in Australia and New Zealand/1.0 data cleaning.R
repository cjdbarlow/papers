...
data = data %>%
    mutate(
        ...
        across(c(siteid, hospitalclassification, tf_cat,
                 ...
                 sofa_bil, sofa_bp, sofa_gcs, sofa_ren, sofa_tot, sofa_ren_nochr, sofa_pf_nochr, sofa_bil_nochr,
               ...
               as.factor),
        # Integer
        across(c(preg_gestwk, age, smokingintensity, inv_hours, niv_hours, gcsverb, gcsmotor,
...
# 3 Error checking
# 3.0 Identify UIDs that may not actually uniquely identify patients
# Test validity of UID as an actual unique identifier by seeing whether patients with the same UID have the same sex
# and (roughly, i.e. within 8 weeks) the same birth day
invalid = data %>%
     mutate(birth = icu_ad_dtm - duration(years = age)) %>%
     group_by(uid) %>%
     select(age, birth, sex) %>%
     mutate(same_sex = n_distinct(sex) == 1,
            birth_range = abs(difftime(first(birth), last(birth), units = "weeks")),
            same_birth = birth_range < 8) %>%
...
## 3.1 Error checking
table(data$hosp_srce, data$prox_adm)
...
dubious = data %>%
    filter(uid %in% dubious$uid)
    # On inspection, it looks like many of the hosp_srce codes are either incorrect, or refer to ICUs not included in the ANZICS APD

saveRDS(dubious, file = "dubious.Rds")
...
## Indigeneity
data = data %>%
    mutate(indigenous = case_when(ind_origin == 1 ~ 1,
                                  ind_origin == 3 ~ 3, # Aboriginal & Torres Strait Islander
                                  ind_origin == 5 ~ 4, # Māori
                                  indigenous == 1 ~ 1, # Aboriginal
                                  nhi_indigenous == "Māori" ~ 4,
                                  TRUE ~ 0) %>%
               as.factor()) %>%       # None of the above
...
subcode_card_complex = c(1208.02, 1208.03, 1208.14, 1208.24)
...
code_msk =          code_list[(code_list > 1100 & code_list < 1200) | (code_list > 1900 & code_list < 200)]
code_other = c(code_gi, code_neuro, code_sepsis, code_trauma, code_metabolic, code_haeme, code_gu, code_msk)
...
data = data %>%
    mutate(dx_arrest = if_any(starts_with("ap3diag"), ~. %in% code_arrest),
           ...
           dx_other = if_any(starts_with("ap3diag"), ~. %in% code_other),
    ...
    mutate(ecmo_subcode_present = ifelse(ap3_subcode %in%
           ...
           ecmo_primarycode_present = ifelse(ap3diag %in%
                                                 c(code_arrest, code_card_shock, code_card_fail, code_card_simple,
                                                   ...
                                                   code_metabolic, code_haeme, code_gu, code_msk, code_other),
                                             TRUE, FALSE),
